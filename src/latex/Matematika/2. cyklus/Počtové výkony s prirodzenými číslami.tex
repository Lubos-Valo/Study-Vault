...
Sčítanie je operácia, pri ktorej spájame dve alebo viac množstiev do jedného celku. Čísla, ktoré sčítavame, sa nazývajú sčítance a výsledok sčítania sa nazýva súčet. Slovom súčet označujeme tiež zápis sčítania. Operáciu sčítania zapisujeme znakom \enquote{\(+\)}. Sčítanie čísla \(a\) a čísla \(b\) zapisujeme ako
\[
a + b.
\]
...
Pre ľahšiu manipuláciu číslic a ich rádov sa pri tomto algoritme využíva zápis čísel pod seba. Dva sčítance sa zapíšu jeden pod druhým tak, aby každý rád sčítancov bol v tom istom stĺpci. Pod druhý sčítanec sa urobí vodorovná čiara, aby sme odlíšili sčítance od súčtu. Výsledné číslice súčtu sa zapisujú pod túto čiaru do príslušných stĺpcov. Nad sčítance sa môže urobiť pomocný riadok pre zapisovanie prenesenej číslice z predošlého medzivýsledku, nie je ale nutný. Znak sčítania zapisujeme nad vodorovnú čiaru pred druhý sčítanec. Tento grafický zápis uľahčuje prácu s jednotlivými číslicami a zabraňuje častým chybám. Príklad zápisu sčítania čísel \(347 + 58\) pod seba:
\begin{table}[H]
  \centering
  \[
  \begin{array}{r}
...
Pri skúmaní vlastností sčítania vychádzame z jej definície. Každé prirodzené číslo môžeme vyjadriť ako súčet jeho predchodcu a jednotky. Napríklad číslo \(5\) môžeme vyjadriť ako \(4 + 1\). Takto môžeme každé prirodzené číslo vyjadriť ako súčet jednotiek, kde počet jednotiek dáva výsledné číslo. Prevod čísla \(5\) na jednotky vypadá nasledovne:
\[
\begin{aligned}
  &5, \\
  &4 + 1, \\
...
\end{aligned}
\]
Týmto môžeme previesť čísla v sčítaní na sčítanie postupnosti jednotiek. Napríklad sčítanie \(2 + 3\) môžeme previesť takto:
\[
\begin{aligned}
  &2 + 3, \\
  &2 + 2 + 1, \\
