# 6. ročník

## Údaje a informácie

### Rastrová grafika

- Tvorba a úprava obrázka v rastrovom tvare

### Vektorová grafika

- Porovnanie rastrových a vektorových obrázkov
- Tvorba a úprava obrázka vo vektorovom tvare

### Prezentácie

- Zásady tvorby prezentácií
- Tvorba a úprava prezentácií

### Animácie

- Tvorba a úprava animácií
- 2D animácie
- 3D animácie

### Videá

- Tvorba a úprava videa
- Nahrávanie videa

### Zvuk

- Tvorba a úprava zvuku a hudby
- Nahrávanie zvuku

### Multimédiá – video, zvuk a text

- Úprava a kombinovanie multimediálneho obsahu
- Kombinovanie videa, zvuku a textu
- Vzťahy medzi grafikou, textom, číslami a zvukom

### Digitalizácia

- Digitalizácia tlačeného textu
- Digitalizácia obrázkov
- Digitalizácia zvuku
- Digitalizácia videa
- Digitalizácia analógového signálu
- Skenovanie a tlačenie
- Získavanie informácií rôznych typov pomocou konkrétnych nástrojov

### Získavanie informácií

- Vyhľadávanie informácií v informačnom systéme a databáze
- Databáza
- Knižnica
- Elektronický obchod
- Rezervácie lístkov

## Technológie

### Počítačový hardvér

- Počítač a typy počítačov
- Komponenty počítača
- Periférie
- Skladanie počítača
- IoT

### Počítačový softvér

- Program
- Operačný systém
- Aplikačný softvér
- Grafické používateľské rozhranie

### Súborový systém

- Správca súborov
- Formát a typ súborov

### Nastavenia a práca s aplikáciami

- Nastavenie zvuku, pracovnej plochy, klávesnice, 
- Prenášanie informácií medzi spustenými aplikáciami
- Nástroje na modelovanie a simulovanie systémov

### Spotrebiče a vnorené systémy

## Digitálna spoločnosť

# 7. ročník

## Údaje a informácie

- Práca s textovými a grafickými údajmi

  - textová informácia
  - grafická informácia
  - neformátovaný a formátovaný text
  - skryté znaky v texte
  - formátovacie nástroje
  - vplyv formátovania na výsledný text
  - operácie s textom
  - obrázok ako súčasť textu
  - odrážky a číslovanie

## Technológie

- Počítačové siete

  - sieť
  - počítačová sieť
  - sieťové zariadenia
  - doména
  - lokálne súbory
  - súbory na sieti
  - cesta ako zápis identifikujúci údaje v sieti
  - internet ako celosvetová počítačová sieť

- Zdieľanie a prenos súborov v sieti

  - zdieľanie súborov
  - kopírovanie súborov
  - prenášanie súborov
  - ukladanie súborov v rámci siete
  - sťahovanie súborov
  - posielanie súborov

- Web a elektronická komunikácia

  - prispievanie do webového obsahu
  - správa
  - e-mail
  - e-mailová adresa
  - e-mailový klient
  - príloha
  - adresár
  - kôš
  - pripájanie prílohy k správe
  - zobrazovanie prijatej prílohy

- Vyhľadávanie a hodnotenie informácií

  - vyhľadávanie informácií
  - výstup vyhľadávania
  - správnosť vyhľadaných informácií
  - kvalita vyhľadaných informácií
  - posudzovanie výsledkov vyhľadávania

## Digitálna spoločnosť

- Škodlivý softvér a vírusy

  - škodlivý softvér
  - vírus ako program
  - schopnosti vírusov
  - antivírus ako program na blokovanie škodlivých činností
  - ochrana pred škodlivým softvérom

- Nebezpečné webové stránky a služby

  - nebezpečné webové stránky
  - nebezpečné služby
  - riziká na internete
  - ochrana pred nebezpečnými stránkami a službami
  - bezpečné správanie na internete

- Spam a bezpečnosť komunikácie

  - spam ako nevyžiadaná správa
  - riziká elektronickej komunikácie
  - ochrana pred spamom
  - bezpečné správanie pri práci s elektronickou poštou

- Hekeri a bezpečnosť

  - činnosť hekerov
  - riziká spojené s neoprávnenými zásahmi
  - ochrana pred nebezpečnými aktivitami
  - bezpečné správanie sa v digitálnom prostredí

- Internet a sociálne siete

  - riziká na internete
  - riziká na sociálnych sieťach
  - vplyv digitálnych technológií na ostatných
  - bezpečné správanie na internete
  - zodpovedné používanie digitálnych technológií

- Digitálne technológie a ich vplyv

  - digitálne technológie ako nástroje pre výpočet
  - digitálne technológie ako nástroje komunikácie
  - digitálne technológie ako nástroje modelovania
  - vplyv digitálnych technológií na ostatných
  - možnosti a riziká digitálnych technológií

### Autorské právo a licencie

# 8. ročník

## Programovanie

- Analýza problému a tvorba plánu riešenia

  - informatický problém v zadaní úlohy
  - platí – neplatí
  - pravdivosť a nepravdivosť tvrdenia/výroku
  - logické spojky a, alebo, nie v neformálnom ponímaní
  - premenná ako označenie
  - údaje, ktoré si treba zapamätať
  - miesta, kde sa treba rozhodovať
  - miesta, kde sa treba opakovať činnosť
  - sekvencia, opakovanie a vetvenie
  - jednoduchá manipulácia s údajmi
  - formulovanie plánu riešenia vlastnými slovami
  - príklady a kontrapríklady
  - uvažovanie o budúcich riešeniach

- Premenné a údaje v programe

  - premenná
  - zapamätanie neštruktúrovaných hodnôt
  - použitie zapamätanej hodnoty
  - pravidlá jazyka pre použitie premennej
  - vstup – program – výsledok
  - jednoduchá manipulácia s údajmi

- Postupnosť príkazov

  - príkaz
  - parameter príkazu
  - postupnosť príkazov
  - zápis postupnosti príkazov
  - zostavovanie programu
  - úprava príkazov
  - spustenie programu

- Cykly a opakovanie

  - opakovanie
  - cyklus
  - počet opakovaní
  - vytváranie programov s daným známym počtom opakovaní
  - vzťah medzi opakovaním a výsledkom programu

- Vetvenie a podmienka

  - vetvenie
  - podmienka
  - podmienený príkaz
  - pravdivosť/nepravdivosť podmienky
  - vytváranie programov s jednoduchou podmienkou
  - rozhodovanie programu podľa podmienky

- Vstup a reakcia programu

  - vstup programu
  - spracovanie vstupu
  - výstup programu
  - čakanie na neznámy vstup
  - vykonanie akcie
  - výstup a následný efekt
  - vytváranie programov reagujúcich na vstup

- Podprogramy

  - podprogram
  - pomenovaná postupnosť príkazov
  - používanie podprogramov
  - vytváranie programov s podprogramami

- Interpretácia a krokovanie programu

  - interpretácia programu
  - simulovanie činnosti vykonávateľa
  - krokovanie programu
  - krokovanie riešenia s premennými
  - krokovanie cyklov
  - krokovanie vetvenia
  - sledovanie vstupu, spracovania a výstupu

- Hľadanie a oprava chýb

  - syntaktická chyba v zápise
  - logická chyba pri realizácii programu
  - chyba v postupnosti príkazov
  - chyba pri práci s premennými
  - chyba v cykle
  - chyba vo vetvení
  - analyzovanie toho, čo sa deje pri chybe
  - hľadanie a oprava chýb vo vlastnom programe

- Hodnotenie a vylepšovanie riešení

  - posudzovanie správnosti riešenia
  - posudzovanie riešiteľnosti úlohy
  - diskusia o tom, či má úloha riešenie
  - návrh vylepšenia riešenia
  - dopĺňanie, dokončovanie a modifikovanie rozpracovaného programu

## Údaje a informácie

- Tabuľky a údajové štruktúry

  - postupnosť
  - tabuľka
  - frekvenčná tabuľka
  - kódovacia tabuľka
  - slovník
  - mriežka
  - bunka
  - adresa bunky ako jej pozícia
  - typy údajov v bunkách – číslo, text
  - bunky so vzorcami

- Výpočty v tabuľke

  - jednoduché výpočty
  - vzorce
  - funkcie pre jednoduché výpočty
  - práca s adresou bunky
  - úprava údajov v tabuľke
  - vytváranie a používanie buniek so vzorcami

- Stromy a grafy

  - stromové štruktúry
  - grafové štruktúry
  - organizovanie informácií do štruktúr
  - interpretovanie údajov zo štruktúr
  - práca s grafovými a stromovými štruktúrami

## Technológie

## Digitálna spoločnosť

- Umelá inteligencia

  - umelá inteligencia
  - možnosti umelej inteligencie
  - obmedzenia umelej inteligencie
  - diskusia o využití umelej inteligencie

- Informatika ako povolanie

  - informatika z pohľadu povolania
  - využitie informatiky v pracovnom živote
  - digitálne technológie v rôznych oblastiach